...
Ta tiếp tục thực hiện cho các node \code{1, 5, 2}  còn lại. Để tiết kiệm không gian, mình sẽ không ghi các node có giá trị âm.
...
\end{forest}
\end{center}
Với những hiệu thu được, ta lại tiếp tục đệ quy, lại lấy chúng trừ cho các phần tử trong \code{a}. Nếu hiệu âm, chúng ta sẽ bỏ qua (mình sẽ không vẽ lại).

\textit{Cụ thể: Ở số 1, do nó không thể trừ cho bất cứ phần tử nào trong \code a được nữa (vì nếu trừ hiệu của chúng sẽ âm) nên mình không vẽ lại.}
...
\begin{itemize}
    \item \code{n < 0}: Nghĩa là ta đã vượt quá giới hạn cho phép trong đệ quy (nhìn vào đồ thị thứ 2, ta có thể thấy trường hợp này: $8 - 2 - 7 = -1$). Lúc này ta sẽ trả về \code{nullptr}.
    \item \code{n = 0}: nghĩa là ở hàm bên ngoài bài toán đã được giải đúng, và giải thuật đệ quy kết thúc tại đây. Lúc này ta sẽ trả về một mảng rỗng (việc này có ý nghĩa nhằm tiết kiệm code trong lúc cài đặt, các bạn có thể tự quy định và tự cài đặt theo kiểu của riêng mình).
\end{itemize}
...
\begin{enumerate}
    ...
    \item Lần lượt lấy \code n trừ cho các phần tử trong \code{a}. Ta tiếp tục gọi hàm đệ quy cho những hiệu số vừa thu được, ghi lại kết quả.
    \item Với mỗi kết quả thu được, nghĩa là với mỗi dãy có tổng bằng \code{n}, ta kiểm tra xem liệu dãy đó có phải là dãy có số lượng phần tử nhỏ nhất không?
    \begin{itemize}
...
Giả sử cho \code{a = \{1, 2, 5, 25 \}} và \code{n = 100}. Nhìn bằng mắt thường, ta có thể thấy ngay dãy ngắn nhất là \code{25 + 25 + 25 + 25}, nhưng với giải thuật ở trên, nó sẽ chạy rất lâu. Máy mình xài \code{AMD Ryzen 5 PRO 4650U} mà nó giải 12 phút rồi vẫn chưa ra. Sau khi áp dụng quy hoạch động, đáp án được giải ra dưới 1 giây.\\
...
\begin{itemize}
    \item Thêm một biến mảng vào tham số của hàm. Mảng này sẽ chứa các lời giải cho các bài toán con (phương pháp này gọi là \code {cache}).
